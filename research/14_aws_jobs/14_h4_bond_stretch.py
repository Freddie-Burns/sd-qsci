"""
Stretch an H4 chain and plot the energy vs bond length.
"""
...
bond_lengths = np.linspace(0.5, 4.0, 36)
rhf_energies = []
uhf_afm_energies = []
uhf_fm_energies = []
fci_energies = []

print(f"Stretching H4 chain from 0.5 to 2.5 Angstrom...")
...
plt.title('H4 Chain: Energy vs Bond Length')
plt.grid(True)
plt.legend()

# Save the figure
plot_path = DATA_DIR / "h4_bond_stretch.png"
plt.savefig(plot_path)
print(f"\nPlot saved to {plot_path}")

# Save the data
data_path = DATA_DIR / "h4_bond_stretch_data.csv"
