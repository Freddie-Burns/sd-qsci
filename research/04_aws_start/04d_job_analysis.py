...
def main():
    ...
    if REORDER_BITSTRINGS:
        tqc_path = out_dir / "qiskit_circuit_transpiled.qpy"
        ...
        for bitstring, count in list(counts.items()):
            ...
            new_bitstring = new_bitstring[::-1] # reverse back to original order
    ...
    # Default: counts vs FCI (may be overwritten below if sim present)
    ...
    # If available, also plot the simulated statevector amplitudes (from local simulation)
    sim_sv_path = base_dir / "data" / DATE_TAG / "sim_statevector.json"
