...
data_dir = Path(
    __file__).parent / 'data' / stem / f"{bond_length:.1f}A"

# Run quantum chemistry calculations
# Define H6 in a triangular lattice
a = bond_length
h = a * np.sqrt(3) / 2
mol = gto.M(
    atom=f'''
...
patterns = {
    # "up down up up up down": [1, -1, 1, 1, 1, -1],  # triplet?
    "up down up down up down": [1, -1, 1, -1, 1, -1],
    "up up up down down down": [1, 1, 1, -1, -1, -1],
    "up down up down down up": [1, -1, 1, -1, -1, 1],
}
